...
### 1. **Unhandled i32 Conversion Panic in Order Status** ⚠️ CRITICAL
**File:** `src/postgresql/dborder.rs` (line 83)  
**Severity:** CRITICAL - Can panic at runtime  

**Issue:**
```rust
// Line 83 in dborder.rs
impl From<i32> for DbOrderStatus {
    fn from(val: i32) -> Self {
        match val {
            1 => DbOrderStatus::New,
            2 => DbOrderStatus::InProgress,
            3 => DbOrderStatus::ReadyToShip,
            4 => DbOrderStatus::Shipping,
            5 => DbOrderStatus::Done,
            _ => panic!("Unknown order status: {}", val),  // PANIC!
        }
    }
}
```

**Problem:** If database contains an invalid order status value, the conversion will panic instead of returning an error. This can happen due to:
- Database corruption
- Concurrent migration issues
- Buggy updates from other services

**Impact:** Application crash in production when fetching orders with corrupted status

**Fix:** Should return `Result<Self, ShopsterError>` instead:
```rust
impl TryFrom<i32> for DbOrderStatus {
    type Error = ShopsterError;
    fn try_from(val: i32) -> Result<Self, ShopsterError> {
        match val {
            1 => Ok(DbOrderStatus::New),
            2 => Ok(DbOrderStatus::InProgress),
            3 => Ok(DbOrderStatus::ReadyToShip),
            4 => Ok(DbOrderStatus::Shipping),
            5 => Ok(DbOrderStatus::Done),
            _ => Err(ShopsterError::InvalidOperationError(
                format!("Unknown order status: {}", val)
            )),
        }
    }
}
...
### 2. **Unwrap on Optional Product Price** ⚠️ CRITICAL
**File:** `src/products.rs` (line 91)  
**Severity:** CRITICAL - Panic when product has no price  

**Issue:**
The conversion from `Product` to `DbProduct` unconditionally unwraps the price field:
```rust
// products.rs, line ~91
pub fn insert(&self, product: &Product) -> Result<Product, ShopsterError> {
    let db_product = DbProduct::from(product);  // Calls From<&Product>
    // ...
}

// In From<&Product> implementation:
price_cents: product.price.as_ref().unwrap().amount,  // PANIC if price is None!
```

**Problem:** Products can be created with `price: None` in the domain model, but the database layer assumes it's always `Some`. This will panic during insert or update.

**Impact:** Application crash when inserting products without prices

**Evidence:** Tests create products with `Some(price)`, but nothing prevents domain code from creating products with `None`

**Fix:** Handle `None` case explicitly:
```rust
price_cents: product.price.as_ref().map(|p| p.amount).unwrap_or(0),
price_currency: product.price.as_ref().map(|p| p.currency.clone()).unwrap_or_else(|| "EUR".to_string()),
```

---

### 3. **Unwrap on Migrations Can Panic** ⚠️ CRITICAL
**File:** `src/lib.rs` (line 177)  
**Severity:** CRITICAL - Migration failures swallowed  

**Issue:**
```rust
// lib.rs, line ~177
let res = run_pending_migrations(&mut conn);
if res.is_err() {
    println!("Migration error: {:?}", res);
    // Continues anyway!
}

// Later, if migrations fail:
pub fn run_pending_migrations(connection: &mut PgConnection) -> MigrationResult<()> {
    MigrationRunner::new(&MIGRATIONS).run(connection) // Can fail
}
```

Combined with other unwrap calls:
```rust
// Line 262 in lib.rs
.lock().unwrap()  // Can poison and panic!
```

**Problem:** If migrations fail, errors are printed but execution continues. Later operations that depend on schema will fail with unclear errors. Additionally, `.lock().unwrap()` on Mutex will panic if the lock is poisoned.

**Impact:** Cryptic errors during database initialization; production data loss risk

**Fix:**
```rust
let res = run_pending_migrations(&mut conn)?;  // Propagate error
// Use lock().map_err() to handle poisoned mutexes gracefully
```
